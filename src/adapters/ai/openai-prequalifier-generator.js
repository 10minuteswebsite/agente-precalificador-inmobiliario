    ...
    properties: {
      ...
      qualification_state: {
        ...
        properties: {
          ...
          appointment_consent: { type: "string", enum: ["pending", "accepted", "declined", "ambiguous"] },
        },
  ...
  return async (input) => {
    ...
    const result = await generate({
      ...
      instructions: input.agent_dna.kind === "real_estate_prequalifier" ? [
        ...
        "Only mark prequalified when the configured required criteria are supported. A prequalified lead is not yet consenting to an appointment: invite the lead naturally and set appointment_consent=pending. Set appointment_consent=accepted only when the latest lead message clearly accepts the invitation; set declined or ambiguous when appropriate. Never transfer to scheduling while consent is pending, declined or ambiguous.",
